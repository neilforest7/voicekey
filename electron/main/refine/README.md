...
- `service.ts` - 无状态 `RefineService`，每次调用都读取最新 `llmRefine` 配置，使用固定 transcript 包装、静态术语表感知、按内容结构酌情分段/按需分点并要求列表项逐行输出的抗注入 prompt 执行润色和连接测试。
...
- `__tests__/service.test.ts` - 覆盖 endpoint 规范化、抗注入 prompt / transcript 包装、静态术语表 prompt 组合、按内容结构酌情分段/按需分点与列表逐行规则、中英混排空格规则、错误映射与连接测试。
